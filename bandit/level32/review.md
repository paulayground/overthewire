...
`hello=world`와 같이 입력했을때, `HELLO=WORLD`로 적용되어 어쨌든 `$HELLO`를 입력했을 때의 결과를 기대했는데, `system()`은 일회용 쉘을 열고 닫아 환경변수가 적용하고 결과를 못 보는 이유를 알았다.
또 `$PWD`같이 이미 등록된 변수들은 볼 수 있었는데, 프로세스가 생성될 때 부모 프로세스의 환경 변수를 가져가서 출력될 수 있었다는 것도 알게됐다.
...
"이건 무슨 힌트를 달라고 하기도 어려웠고... 힌트 없으면 못 깼을 것 같다"
